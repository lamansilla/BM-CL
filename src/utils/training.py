...
METHODS = {"worst_acc", "balanced_acc", "worst_tpr", "balanced_tpr"}
...
def train(
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        base_metric = "tpr" if "tpr" in method else "acc"
        group_scores = evaluate_by_group(model, val_loader, device, metric=base_metric)
        group_values = list(group_scores.values())
        group_str = " ".join([f"{val:.3f}" for val in group_values])

        if method in ("worst_acc", "worst_tpr"):
            val_score = min(group_values)
        elif method in ("balanced_acc", "balanced_tpr"):
        ...
        print(
            f"[Epoch {epoch}] train_loss: {run_loss:.3f}, val_group_{base_metric}: {group_str}, "
    ...
    df.to_csv(history_path or os.path.join(log_dir, "history.csv"), index=False)
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            for g_id in torch.unique(g):
                ...
                correct_by_group[g_id.item()] += (y_pred == y_true).sum().item()
                total_by_group[g_id.item()] += mask.sum().item()
                tp_by_group[g_id.item()] += ((y_pred == 1) & (y_true == 1)).sum().item()
                pos_by_group[g_id.item()] += (y_true == 1).sum().item()

    if metric == "acc":
        return {g: correct_by_group[g] / total_by_group[g] for g in correct_by_group}
    elif metric == "tpr":
        return {
            g: tp_by_group[g] / pos_by_group[g] if pos_by_group[g] > 0 else 0.0
            for g in correct_by_group
        }
...
def partition_groups(results, metric="acc"):
    ...
    for g_id in group_ids:
        ...
        if metric == "tpr":
            ...
            val = tp / pos if pos > 0 else 0.0
        else:
            val = (preds[mask] == labels[mask]).sum() / mask.sum()
        group_metrics[g_id] = val

    vals = list(group_metrics.values())
    threshold = sum(vals) / len(vals)
    best_groups = [g for g, v in group_metrics.items() if v > threshold]

    worst_groups = [g for g in group_metrics if g not in best_groups]

    balanced_val = sum(vals) / len(vals)
    return {
        "best_groups": best_groups,
        "worst_groups": worst_groups,
        "group_ids": list(group_ids),
        "group_metrics": [group_metrics[g] for g in group_ids],
